...
import com.ttterbagames.businesssimulator.databinding.FragmentStockInfoBinding
...
class StockInfoFragment(var s: Stock) : Fragment() {

    lateinit var binding: FragmentStockInfoBinding
    ...
    fun addObservers() {
        s.priceChangeNotifier.observe(activity as LifecycleOwner, {
            ...
            if (s.lastDiff >= 0.0) {
                binding.tvDiff.text = Strings.get(R.string.price_diff_positive, s.lastDiff, s.lastDiffPercent.absoluteValue)
                binding.tvDiff.setTextColor(Color.parseColor("#05C46B"))
            } else {
                binding.tvDiff.text = Strings.get(R.string.price_diff_negative, s.lastDiff.absoluteValue, s.lastDiffPercent.absoluteValue)
                binding.tvDiff.setTextColor(Color.parseColor("#e74c3c"))
            }
            binding.tvCapitalization.text = Strings.get(R.string.money, (s.currentPrice * s.lotsCount))
            setGraphAxis()
            setTimeAxis()
        })
    }
    ...
    fun initStaticViews() {
        binding.apply {
            ...
            tvCapitalization.text = Strings.get(R.string.money, (s.currentPrice * s.lotsCount))
            tvDividendYield.text = Strings.get(R.string.percent, s.dividendPercent)
            tvAvailableShares.text = Strings.get(R.string.integer_value, (s.lotsCount - s.soldLotsCount))
        }
    }
    ...
    fun setButtonListeners() {
        ...
        binding.btnBuy.setOnClickListener {
            requireActivity().supportFragmentManager.commit {
                ...
                replace(R.id.fl_wrapper, StockBuyFragment(s))
                addToBackStack(null)
            }
        }
    }
